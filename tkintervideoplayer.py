...
class TkinterVideo(tk.Label):
    ...
    def __init__(self, master, scaled: bool = True, consistant_frame_rate: bool = True, keep_aspect: bool = False,
        ...
        self.consistant_frame_rate = consistant_frame_rate  # tries to keep the frame rate consistent by skipping over a few frames
        ...
        self._seek = False
        self._seek_sec = 0.0  # now supports float seconds

        self._video_info = {
            "duration": 0,  # duration of the video
            "framerate": 0,  # frame rate of the video
            "framesize": (0, 0)  # tuple containing frame width and height of the video
        }
    ...
    def _load(self, path):
        ...
        try:
            # Open without passing invalid options
            with av.open(path) as self._container:
                ...
                while self._load_thread == current_thread and not self._stop:
                    if self._seek:
                        # Use float seconds converted to int microseconds for seeking
    ...
    def seek(self, sec: float):
        """Seeks to specific time (sec can be a float)."""
        self._seek = True
        self._seek_sec = sec
...
if __name__ == "__main__":
    def loop(e):
        # If the video had ended then replay the video from the beginning
        tkvideo.play()
    ...
    tkvideo.bind("<<Ended>>", loop)  # When the video ends, call the loop function
    root.mainloop()
